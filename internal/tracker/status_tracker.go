...
func (t *StatusTracker) TrackIssueState(issueID int, newState string) (bool, error) {
	var currentState sql.NullString
	var previousState sql.NullString
	err := t.db.QueryRow(
		"SELECT state, previous_state FROM issues WHERE id = ?",
		issueID,
	).Scan(&currentState, &previousState)
	...
	if currentStateValue != newState {
		_, err = t.db.Exec(
			"UPDATE issues SET state = ?, previous_state = ?, updated_at = ? WHERE id = ?",
			newState, currentStateValue, time.Now(), issueID,
		)
		if err != nil {
			return false, fmt.Errorf("Issue状態の更新に失敗しました: %w", err)
		}

		t.logger.Info("Issue状態が変更されました",
...
func (t *StatusTracker) UpdateIssueLastSynced(issueID int) error {
	_, err := t.db.Exec(
		"UPDATE issues SET last_synced_at = ? WHERE id = ?",
		time.Now(), issueID,
	)
	return err
}
